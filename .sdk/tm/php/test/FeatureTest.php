...
class FtCtrl extends N4chanControl
{
...
class FtClient
{
    ...
    private N4chanUtility $utility;

    public function __construct(N4chanUtility $utility, array $options)
    {
    ...
    public function get_utility(): N4chanUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (N4chanContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (N4chanContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = N4chanConfig::make_feature($name);
    ...
    private static function buildUrl(N4chanSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof N4chanError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new N4chanResponse($fetched);
        }
    ...
    private function fail(N4chanContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (N4chanContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
