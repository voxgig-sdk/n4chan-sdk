...
boru-driven command-line client **and** interactive REPL for the N4chan
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/n4chan-cli)
make build
...
./n4chan-cli list board

# 5. Override the API base URL for a single call
N4CHAN_BASE=https://api.example.com ./n4chan-cli list archive
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/n4chan-cli
   ```
...
Run with no arguments to open a REPL (prompt `n4chan>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `archive`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/n4chan-cli`. |
...
(`./n4chan-cli <expr>`) and interactively (the REPL), and expressions compose the
