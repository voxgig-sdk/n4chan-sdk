...
MCP server exposing the N4chan SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `n4chan_list` | `entity`, optional `query` map | First page of records as JSON |
| `n4chan_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `n4chan_list` and
`n4chan_load` tools become available in new sessions.
