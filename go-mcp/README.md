...
[MCP](https://modelcontextprotocol.io) server exposing the N4chan SDK as
two agent tools — `n4chan_list` and `n4chan_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/n4chan-mcp)
make build

# 2. Provide credentials via the environment
export N4CHAN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user n4chan \
  -- /absolute/path/to/n4chan-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./n4chan-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// n4chan_list: first page of records
{ "entity": "archive" }
{ "entity": "archive", "query": { } }

// n4chan_load: one record by id
{ "entity": "archive", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/n4chan-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user n4chan \
     -- "$PWD"/dist/*/n4chan-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `n4chan_list` and `n4chan_load` tools now appear
   in new sessions. Ask the agent to *"list archive using n4chan"*
   and it calls `n4chan_list` with `{"entity":"archive"}`.
...
```sh
export N4CHAN_APIKEY=sk_live_xxx            # API key
export N4CHAN_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `n4chan_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `n4chan_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `N4CHAN_APIKEY` | API key sent with every request. |
| `N4CHAN_BASE` | Optional override of the API base URL. |
...
```sh
./n4chan-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"n4chan_load","arguments":{"entity":"archive","query":{"id":1}}}}'
```
