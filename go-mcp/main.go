// Command N4chan-mcp is an MCP server exposing the N4chan
// SDK as MCP tools.
//
// Two tools are registered:
//
//	N4chan_list  — list records of an entity
//	N4chan_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: N4CHAN_APIKEY carries the API key and
	// N4CHAN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("N4CHAN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("N4CHAN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewN4chanSDK(opts)
