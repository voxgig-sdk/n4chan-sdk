...
func TestCatalogEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Catalog(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Catalog(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set N4CHAN_TEST_CATALOG_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		catalogRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.catalog")))
...
func catalogBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"catalog01", "catalog02", "catalog03", "board01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("N4CHAN_TEST_CATALOG_ENTID")
	...
	idmapResolved := core.ToMapAny(env["N4CHAN_TEST_CATALOG_ENTID"])
