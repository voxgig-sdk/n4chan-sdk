...
import (
	...
	omni "github.com/voxgig-sdk/n4chan-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.N4chanError); is {
				out := map[string]any{
					"name":    "N4chanError",
...
func omniCtx(arg any, client *sdk.N4chanSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.N4chanSDK); is {
			client = live
