...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("N4CHAN_TEST_LIVE") == "TRUE" ||
		os.Getenv("N4CHAN_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("N4CHAN_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.N4chanSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.N4chanError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.N4chanSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.N4chanError{Msg: msg, Code: code}
}
