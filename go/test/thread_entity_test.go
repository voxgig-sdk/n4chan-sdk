...
func TestThreadEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Thread(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Thread(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set N4CHAN_TEST_THREAD_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func threadBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("N4CHAN_TEST_THREAD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["N4CHAN_TEST_THREAD_ENTID"])
