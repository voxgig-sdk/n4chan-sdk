...
The Lua SDK for the N4chan API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Archive(nil):list(nil, nil)
...
local result, err = client:N4chan(nil):load(
  { id = "test01" }, nil
)
