...
local ArchiveEntity = {}
ArchiveEntity.__index = ArchiveEntity


function ArchiveEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArchiveEntity)
  self._name = "archive"
  self._client = client
...
function ArchiveEntity:get_name()
  return self._name
end


function ArchiveEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArchiveEntity.new(self._client, opts)
end
...
function ArchiveEntity:mark_deleted()
  self._deleted = true
end


function ArchiveEntity:deleted()
  return true == self._deleted
end


function ArchiveEntity:data_set(args)
  if args ~= nil then
...
function ArchiveEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArchiveEntity:match_set(args)
  if args ~= nil then
...
function ArchiveEntity:match_get()
...
function ArchiveEntity:stream(action, args, callopts)
...
function ArchiveEntity:_run_op(ctx, post_done)
  local utility = self._utility
