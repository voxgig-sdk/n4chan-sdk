...
local BoardEntity = {}
BoardEntity.__index = BoardEntity


function BoardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BoardEntity)
  self._name = "board"
  self._client = client
...
function BoardEntity:get_name()
  return self._name
end


function BoardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BoardEntity.new(self._client, opts)
end
...
function BoardEntity:mark_deleted()
  self._deleted = true
end


function BoardEntity:deleted()
  return true == self._deleted
end


function BoardEntity:data_set(args)
  if args ~= nil then
...
function BoardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BoardEntity:match_set(args)
  if args ~= nil then
...
function BoardEntity:match_get()
...
function BoardEntity:stream(action, args, callopts)
...
---@param reqmatch BoardListMatch
---@param ctrl? table
---@return Board[]
---@return string? err
function BoardEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function BoardEntity:_run_op(ctx, post_done)
  local utility = self._utility
