...
local CatalogEntity = {}
CatalogEntity.__index = CatalogEntity


function CatalogEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CatalogEntity)
  self._name = "catalog"
  self._client = client
...
function CatalogEntity:get_name()
  return self._name
end


function CatalogEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CatalogEntity.new(self._client, opts)
end
...
function CatalogEntity:mark_deleted()
  self._deleted = true
end


function CatalogEntity:deleted()
  return true == self._deleted
end


function CatalogEntity:data_set(args)
  if args ~= nil then
...
function CatalogEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CatalogEntity:match_set(args)
  if args ~= nil then
...
function CatalogEntity:match_get()
...
function CatalogEntity:stream(action, args, callopts)
...
---@param reqmatch CatalogListMatch
---@param ctrl? table
---@return Catalog[]
---@return string? err
function CatalogEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CatalogEntity:_run_op(ctx, post_done)
  local utility = self._utility
