...
class BoardEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BoardEntity($this->_client, $opts);
    }

    /**
     * @param Board|array $args Board data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = N4chanHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Board|array The current Board data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Board fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = N4chanHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Board fields).
     */
    ...
    /**
     * List Board items matching the given filter.
     *
     * @param BoardListMatch|array|null $reqmatch Match filter (any subset
     *   of Board fields) as an assoc-array; BoardListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Board[]|array A list of Board items as assoc-arrays at
     *   the SDK boundary; throws N4chanError on failure (item-5 convention).
     */
