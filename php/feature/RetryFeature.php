...
class N4chanRetryFeature extends N4chanBaseFeature
{
    ...
    public function init(N4chanContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (N4chanContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(N4chanContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(N4chanContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
