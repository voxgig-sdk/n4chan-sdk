...
class N4chanSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = N4chanHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = N4chanHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!N4chanFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, N4chanFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return N4chanUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = N4chanHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = N4chanHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = N4chanHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new N4chanError($op . "_allow",
                "N4chanSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = N4chanHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new N4chanError("graphql_error",
                "N4chanSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
