...
class BoardEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set N4CHAN_TEST_BOARD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $board_ref01_data = null;
        if (count($board_ref01_data_raw) > 0) {
...
function board_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["board01", "board02", "board03"] as $k) {
    ...
    $entid_env_raw = getenv("N4CHAN_TEST_BOARD_ENTID");
