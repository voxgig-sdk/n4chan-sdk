...
class CatalogEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Catalog(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Catalog(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set N4CHAN_TEST_CATALOG_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function catalog_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["catalog01", "catalog02", "catalog03", "board01"] as $k) {
    ...
    $entid_env_raw = getenv("N4CHAN_TEST_CATALOG_ENTID");
