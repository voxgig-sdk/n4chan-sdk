...
//    only - a typed N4chanContext there would make every ctx
...
final class N4chanOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new N4chanContext($clean, null);
    ...
    public static function err_from_map($m): ?N4chanError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new N4chanError(is_string($code) ? $code : '', $msg);
    }
