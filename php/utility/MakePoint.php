...
// N4chan SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class N4chanMakePoint
{
    public static function call(N4chanContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof N4chanError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = N4chanHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = N4chanHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
