...
from n4chan_sdk.n4chan_types import (
    Archive,
    ArchiveListMatch,
)
...
class ArchiveEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ArchiveEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Archive]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Archive().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
