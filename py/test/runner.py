...
class N4chanTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            N4chanTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = N4chanTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = N4chanTestRunner.getenv("N4CHAN_TEST_LIVE")
        override = N4chanTestRunner.getenv("N4CHAN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = N4chanTestRunner.getenv(key)
        ...
        explain = N4chanTestRunner.getenv("N4CHAN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["N4CHAN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if N4chanTestRunner._test_control is not None:
            return N4chanTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                N4chanTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            N4chanTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = N4chanTestRunner.load_test_control()
...
def load_env_local():
    N4chanTestRunner.load_env_local()


def env_override(m):
    return N4chanTestRunner.env_override(m)


def entity_data(v):
    return N4chanTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return N4chanTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return N4chanTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return N4chanTestRunner.load_test_control()


def live_client_options():
    return N4chanTestRunner.live_client_options()


def live_delay_ms():
    return N4chanTestRunner.live_delay_ms()
