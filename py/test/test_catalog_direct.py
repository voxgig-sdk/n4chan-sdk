...
class TestCatalogDirect:
    ...
    def test_should_direct_list_catalog(self):
        setup = _catalog_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-catalog", "live" if setup["live"] else "unit")
        if _skip:
...
def _catalog_direct_setup(mockres):
