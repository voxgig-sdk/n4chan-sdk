...
class TestThreadDirect:

    def test_should_direct_list_thread(self):
        setup = _thread_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-thread", "live" if setup["live"] else "unit")
        if _skip:
...
def _thread_direct_setup(mockres):
