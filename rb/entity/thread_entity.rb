...
class ThreadEntity
  ...
  def make
    opts = @_entopts.dup
    ThreadEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = N4chanHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Thread, Hash] the current Thread data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = N4chanHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Thread fields)
  def match_get
  ...
  # List Thread items matching the given filter.
  #
  # @param reqmatch [ThreadListMatch, Hash, nil] match filter (any subset of
  #   Thread fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Thread>, Array] the matching Thread items; raises N4chanError on failure
