...
class ArchiveDirectTest < Minitest::Test
  def test_direct_list_archive
    setup = archive_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-archive", setup[:live] ? "live" : "unit")
    if _should_skip
...
def archive_direct_setup(mockres)
