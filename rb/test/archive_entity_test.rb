...
class ArchiveEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = N4chanSDK.test(seed, nil)
    seen = base.Archive(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Archive(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = archive_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "archive." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    archive_ref01_data = nil
    if archive_ref01_data_raw.length > 0
      archive_ref01_data = Helpers.to_map(archive_ref01_data_raw[0][1])
    end

    # LIST
    archive_ref01_ent = client.Archive(nil)
    archive_ref01_match = {
...
def archive_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "archive", "ArchiveTestData.json")
