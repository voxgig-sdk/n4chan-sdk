...
class BoardDirectTest < Minitest::Test
  def test_direct_list_board
    setup = board_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-board", setup[:live] ? "live" : "unit")
    if _should_skip
...
def board_direct_setup(mockres)
