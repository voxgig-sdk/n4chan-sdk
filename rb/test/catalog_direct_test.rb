...
class CatalogDirectTest < Minitest::Test
  def test_direct_list_catalog
    setup = catalog_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-catalog", setup[:live] ? "live" : "unit")
    if _should_skip
...
def catalog_direct_setup(mockres)
