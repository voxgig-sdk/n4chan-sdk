...
class ThreadDirectTest < Minitest::Test
  def test_direct_list_thread
    setup = thread_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-thread", setup[:live] ? "live" : "unit")
    if _should_skip
...
def thread_direct_setup(mockres)
