...
describe('ThreadEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'thread.' + op, live)) return
    }
    ...
    const thread_ref01_list = await thread_ref01_ent.list(thread_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['thread01','thread02','thread03','thread01','thread02','thread03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['N_CHAN_TEST_THREAD_ENTID']
  ...
  idmap = env['N_CHAN_TEST_THREAD_ENTID']
