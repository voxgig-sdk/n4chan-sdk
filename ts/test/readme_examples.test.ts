...
//     (\`new N4chanSDK(...)\` / \`N4chanSDK.test(...)\` become
//     \`N4chanSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new N4chanSDK(...)\` and any
// \`N4chanSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+N4chanSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/N4chanSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
